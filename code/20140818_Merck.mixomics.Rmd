...
<!--merge igraph object-->
```{r, include=FALSE}
  graphLS <- c("net1f.v2", "net1c.v2", "net15f.v2", "net15c.v2", "net16f.v2")
  graphUnion <- gR
  V(graphUnion)$name <- V(graphUnion)$label
  for (graphName in graphLS) {
    eval(parse(text = paste("g <-", graphName)))
    V(g)$name <- V(g)$label
    g1v <- get.data.frame(graphUnion, what = "vertices")
    g2v <- get.data.frame(g, what = "vertices")
    guv <- rbind(g1v, g2v)
    guv <- guv[!duplicated(guv[, "name"]), ]
    g1e <- get.data.frame(graphUnion, what = "edges")
    g2e <- get.data.frame(g, what = "edges")
    gue <- rbind(g1e, g2e)
    graphUnion <- graph.data.frame(gue, directed = FALSE, vertices = guv)
  }
  # position nodes
  label <- V(graphUnion)$label
  labelPos <- matrix(0, ncol = 2, nrow = length(label))
  rownames(labelPos) <- label
  labelPos[label %in% clusterDF[[1]], 1] <- -1
  labelPos[label %in% clusterDF[[1]], 2] <- 1
  labelPos[label %in% clusterDF[[16]], 1] <- 1
  labelPos[label %in% clusterDF[[16]], 2] <- 1
  gene15 <- fData(eset)[listDF, "Gene.Symbol"]
  gene15[gene15 %in% "---"] <- fData(eset)[listDF[gene15 %in% "---"],
                                           "Gene.Title"]
  labelPos[label %in% gene15, 1] <- 0
  labelPos[label %in% gene15, 2] <- 1
  labelPos[label %in% colnames(cytoDF), 1] <- 1
  labelPos[label %in% colnames(cytoDF), 2] <- -1
  flag <- rowSums(labelPos == 0) == 2
  labelPos[flag, 1] <- -1
  labelPos[flag, 2] <- -1
 plot(graphUnion,
           vertex.shape = "circle",
           ...
           edge.color   = "black",
           layout       = labelPos + rnorm(nrow(labelPos) * 2, 0,0.1))
```